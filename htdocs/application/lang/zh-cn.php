<?php

return [
    'Yes'=>'是',
    'No'=>'否',

    'All'=>'全部',

    'Member'=>'会员',
    'Employee'=>'员工',

    'Balance'=>'余额',
    'Credit'=>'积分',
    'Reward'=>'奖励',

    'System Opt.'=>'系统操作',
    'Consume'=>'消费',
    'Recharge'=>'充值',

    'Unknown'=>'未知',

    'Audited'=>'已审核',
    'Hidden'=>'已隐藏',
    'Unaudited'=>'待审核',
    'Confirmed'=>'已确认',
    'Invalid'=>'已失效',
    'Shown'=>'已显示',

    'Cancelled'=>'已取消',
    'Unpaid'=>'待支付',
    'Unshipped'=>'待发货',
    'Unreceived'=>'待收货',
    'Unevaluate'=>'待评价',
    'Completed'=>'已完成',

    'Normal'=>'普通',
    'Top'=>'置顶',
    'Hot'=>'热门',
    'Recommend'=>'推荐',

    'Common Settings'=>'通用配置',
    'Member Settings'=>'会员配置',
    'Third Settings'=>'第三方配置',
    'Advance Settings'=>'高级配置',

    'Text Field'=>'单行文本',
    'Numberic'=>'数字',
    'True False'=>'布尔',
    'Radio Box'=>'单选',
    'Check Box'=>'多选',
    'Select'=>'下拉选择',
    'Textarea'=>'多行文本',
    'Location Picker'=>'位置选择',
    'Upload Image'=>'上传图片',
    'Editor'=>'编辑器',

    'Wechat Pay'=>'微信支付',
    'Alipay '=>'支付宝支付',
    'Unionpay'=>'银联支付',

    'Wechat'=>'微信',
    'Alipay'=>'支付宝',
    'Unioncard'=>'银联卡',

    'Access Denied!'=>'禁止访问！',
    'Bad Request!'=>'非法请求！',
    'Arguments error!'=>'参数错误！',

    'Sign in'=>'登录',
    'Sign up'=>'注册',
    'Sign out'=>'安全退出',
    'Profile'=>'个人资料',
    'Settings'=>'系统设置',

    'Username'=>'用户名',
    'Mobile'=>'手机号',
    'Email'=>'邮 箱',
    'Password'=>'密 码',
    'Verify'=>'验证码',

    'Please login first!'=>'请先登录！',
    'Invalid account!'=>'账号已失效！',
    'The account has login in other places!'=>'该账号已在其它地方登录！',
    'You have no permission to do this operation!'=>'您无权进行此操作！',
    'Bad permission settings, pls contact the manager!'=>'权限设置有误，请联系管理员',
    'Login error of too many times!'=>'登录错误次数过多！',
    'Please fill in the login field!'=>'请填写登录信息！',

    'Login success!'=>'登陆成功！',
    'Account is disabled!'=>'账号已禁用！',
    'Account is disabled, pls contact the super master!'=>'账号被禁用，请联系超级管理员 :(',
    'Account or password increct!'=>'账号或密码错误 :(',
    'Incorrect password: %s'=>'密码错误: %s',
    'Verify code error!'=>'验证码错误！',

    'Add'=>'添加',
    'Save'=>'保存',
    'Edit'=>'编辑',
    'Update'=>'更新',
    'Delete'=>'删除',
    'Add success!'=>'添加成功！',
    'Update success!'=>'保存成功！',
    'Delete success!'=>'删除成功！',
    'Add failed!'=>'添加失败！',
    'Update failed!'=>'保存失败！',
    'Delete failed!'=>'删除失败！',
    'Operation failed!'=>'操作失败！',
    'Operation success!'=>'操作成功！',
];