...
<block name="body">
<include file="public/bread" menu="credit_shop_goods_index" title="商品列表" />
<div id="page-wrapper">
	...
	<div class="row list-header">
		<div class="col-md-6">
			<div class="btn-toolbar list-toolbar" role="toolbar" aria-label="Toolbar with button groups">
				...
				<a href="{:url('credit_shop.goods/add')}" class="btn btn-outline-primary btn-sm mr-2"><i class="ion-md-add"></i> 添加商品</a>
		...
		<div class="col-md-6">
			<form action="{:url('credit_shop.goods/index')}" method="post">
				<div class="form-row">
		...
		<empty name="lists">{:list_empty(10)}</empty>
			<foreach name="lists" item="v" >
				<tr>
					...
					<td class="operations">
					<a class="btn btn-outline-primary btn-sm" href="{:url('credit_shop.goods/edit',array('id'=>$v['id']))}" title="编辑"><i class="ion-md-create"></i> </a>
						<a class="btn btn-outline-primary btn-sm" href="{:url('credit_shop.goods/imagelist',array('aid'=>$v['id']))}" title="图集"><i class="ion-md-images"></i> </a>
						<!--a class="btn btn-outline-dark btn-sm" href="{:url('credit_shop.goods/comments',array('aid'=>$v['id']))}"><i class="ion-md-chatboxes"></i> 评论</a-->
					<a class="btn btn-outline-danger btn-sm link-confirm" href="{:url('credit_shop.goods/delete',array('id'=>$v['id']))}" title="删除" data-confirm="您真的确定要删除吗？\n\n删除后将不能恢复!" ><i class="ion-md-trash"></i> </a>
					</td>
				</tr>
			</foreach>
		</tbody>
		...
		(function(w){
			w.actionPublish=function(ids){
				dialog.confirm('确定将选中商品发布到前台？',function() {
				    $.ajax({
						url:"{:url('credit_shop.goods/push',['id'=>'__id__','type'=>1])}".replace('__id__',ids.join(',')),
						type:'GET',
						dataType:'JSON',
            ...
            w.actionCancel=function(ids){
                dialog.confirm('确定取消选中商品的发布状态？',function() {
                    $.ajax({
                        url:"{:url('credit_shop.goods/push',['id'=>'__id__','type'=>0])}".replace('__id__',ids.join(',')),
                        type:'GET',
                        dataType:'JSON',
                        success:function(json){
                            if(json.code==1){
            ...
            w.actionDelete=function(ids){
                dialog.confirm('确定删除选中的商品？',function() {
                    $.ajax({
                        url:"{:url('credit_shop.goods/delete',['id'=>'__id__'])}".replace('__id__',ids.join(',')),
                        type:'GET',
                        dataType:'JSON',
                        success:function(json){
                            if(json.code==1){
			...
			w.actionSetIncrement=function () {
				dialog.prompt('请输入新的起始ID',function (input) {
					$.ajax({
						url:"{:url('credit_shop.goods/set_increment',['incre'=>'__INCRE__'])}".replace('__INCRE__',input),
						type:'GET',
						dataType:'JSON',
						success:function(json){
							if(json.code==1){
