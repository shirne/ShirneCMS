...
use addon\credit_shop\admin\validate\GoodsValidate;
...
class GoodsController extends BaseController
{
    public function index($key='',$cate_id=0){
        ...
        if($cate_id>0){
            $model->whereIn('goods.cate_id',CategoryFacade::getSubCateIds($cate_id));
        }
        ...
        if ($this->request->isPost()) {
            ...
            if (!$validate->check($data)) {
                ...
                if ($model['id']) {
                    ...
                    $this->success("添加成功", url('credit_shop.goods/index'));
                } else {
        ...
        if ($this->request->isPost()) {
            ...
            if (!$validate->check($data)) {
                ...
                if ($model->allowField(true)->save($data)) {
                    ...
                    $this->success("编辑成功", url('credit_shop.goods/index'));
                } else {
                    delete_image($data['image']);
                    $this->error("编辑失败");
                }
    ...
    public function delete($id)
    {
        ...
        if($result){
            ...
            $this->success("删除成功", url('credit_shop.goods/index'));
        }else{
            $this->error("删除失败");
        }
    }
    public function push($id,$type=0)
    {
        ...
        if ($result && $data['status'] === 1) {
            ...
            $this -> success("上架成功", url('credit_shop.goods/index'));
        } elseif ($result && $data['status'] === 0) {
            user_log($this->mid,'cancelgoods',1,'下架商品 '.$id ,'manager');
            $this -> success("下架成功", url('credit_shop.goods/index'));
        } else {
            $this -> error("操作失败");
        }
    }
    ...
    public function imageadd($aid){
        if ($this->request->isPost()) {
            ...
            if (!$validate->check($data)) {
                ...
                $model = Db::name("GoodsImages");
                $url=url('credit_shop.goods/imagelist',array('aid'=>$aid));
    ...
    public function imageupdate($id)
    {
        ...
        if ($this->request->isPost()) {
            ...
            if (!$validate->check($data)) {
                ...
                $model = Db::name("GoodsImages");
                $url=url('credit_shop.goods/imagelist',array('aid'=>$data['goods_id']));
                $delete_images=[];
                $uploaded=$this->upload('goods','upload_image');
                if(!empty($uploaded)){
                    $data['image']=$uploaded['url'];
    ...
    public function imagedelete($aid,$id)
    {
        ...
        if($result){
            $this->success("删除成功", url('credit_shop.goods/imagelist',array('aid'=>$aid)));
        }else{
            $this->error("删除失败");
        }
    }
