			...
			<div class="card side-cate">
				...
				<ul class="list-group list-group-flush">
					<li class="list-group-item"><a class="list-cate-item"
							href="{:url('index',['key'=>$keyword,'cate_id'=>0])}" data-value="0">不限分类</a></li>
					{foreach $category as $key => $v}
					<li class="list-group-item{$cate_id == $v['id']?' active':''}">
						<a href="javascript:" data-id="{$v['id']}" title="删除分类"
							class="float-right ml-2 text-danger delcate"><i class="ion-md-trash"></i></a>
						<a href="javascript:" data-pid="{$v['id']}" title="添加子类" class="float-right ml-2 addcate"><i
								class="ion-md-add"></i></a>
						<a href="javascript:" data-id="{$v['id']}" title="编辑分类" data-title="{$v['title']}"
							data-short="{$v['short']}" data-name="{$v['name']}" data-sort="{$v['sort']}"
							data-pid="{$v['pid']}" class="float-right addcate"><i class="ion-md-create"></i></a>
				...
				<div class="col-md-6">
					<div class="btn-toolbar list-toolbar" role="toolbar" aria-label="Toolbar with button groups">
						...
						<div class="btn-group btn-group-sm mr-2" role="group" aria-label="action button group">
							...
							<a href="javascript:" class="btn btn-outline-secondary action-btn"
								data-action="cancel">撤销</a>
							<a href="javascript:" class="btn btn-outline-secondary action-btn"
								data-action="delete">{:lang('Delete')}</a>
						</div>
						<a href="{:url('shop.help/add',['cid'=>$cate_id])}"
							class="btn btn-outline-primary btn-sm mr-2"><i class="ion-md-add"></i> 添加帮助</a>
					...
					<tr>
						...
						<td class="operations">
							<a class="btn btn-outline-primary" title="编辑"
								href="{:url('shop.help/edit',array('id'=>$v['id'],'cid'=>$cate_id))}"><i
									class="ion-md-create"></i> </a>
							<a class="btn btn-outline-danger link-confirm" title="{:lang('Delete')}"
								data-confirm="您真的确定要删除吗？\n删除后将不能恢复!"
								href="{:url('shop.help/delete',array('id'=>$v['id']))}"><i class="ion-md-trash"></i>
							</a>
						</td>
					</tr>
					{/volist}
