	...
	<div class="row list-header">
		<div class="col-md-6">
			<div class="btn-toolbar list-toolbar" role="toolbar" aria-label="Toolbar with button groups">
				...
				<a href="{:url('article.index/add')}" class="btn btn-outline-primary btn-sm mr-2"><i
						class="ion-md-add"></i> 添加文章</a>
		...
		<div class="col-md-6">
			<form action="{:url('article.index/index')}" method="post">
				<div class="form-row">
					...
					<td><a href="{:url(' index/article/view',['id'=>$v['id']])}" target="_blank">{$v.title}</a> </td>
								...
								<td class="operations">
									<a class="btn btn-outline-primary" title="编辑"
										href="{:url('article.index/edit',array('id'=>$v['id']))}"><i
											class="ion-md-create"></i> </a>
									<a class="btn btn-outline-primary" title="图集"
										href="{:url('article.index/imagelist',array('aid'=>$v['id']))}"><i
											class="ion-md-images"></i> </a>
									<a class="btn btn-outline-primary" title="评论"
										href="{:url('article.index/comments',array('aid'=>$v['id']))}"><i
											class="ion-md-chatboxes"></i> </a>
									<a class="btn btn-outline-danger link-confirm" title="{:lang('Delete')}"
										data-confirm="您真的确定要删除吗？\n删除后将不能恢复!"
										href="{:url('article.index/delete',array('id'=>$v['id']))}"><i
						...
						(function (w) {
							w.actionPublish = function (ids) {
								dialog.confirm('确定将选中文章发布到前台？', function () {
									$.ajax({
										url: "{:url('article.index/status',['id'=>'__id__','status'=>1])}".replace('__id__', ids.join(',')),
										type: 'GET',
										dataType: 'JSON',
							...
							w.actionCancel = function (ids) {
								dialog.confirm('确定取消选中文章的发布状态？', function () {
									$.ajax({
										url: "{:url('article.index/status',['id'=>'__id__','status'=>0])}".replace('__id__', ids.join(',')),
										type: 'GET',
										dataType: 'JSON',
										success: function (json) {
											if (json.code == 1) {
							...
							w.actionDelete = function (ids) {
								dialog.confirm('确定删除选中的文章？', function () {
									$.ajax({
										url: "{:url('article.index/delete',['id'=>'__id__'])}".replace('__id__', ids.join(',')),
										type: 'GET',
										dataType: 'JSON',
										success: function (json) {
											if (json.code == 1) {
							...
							w.actionSetIncrement = function () {
								dialog.prompt('请输入新的起始ID', function (input) {
									$.ajax({
										url: "{:url('article.index/set_increment',['incre'=>'__INCRE__'])}".replace('__INCRE__', input),
										type: 'GET',
										dataType: 'JSON',
										success: function (json) {
											if (json.code == 1) {
