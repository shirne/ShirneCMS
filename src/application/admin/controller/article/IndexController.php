...
class IndexController extends BaseController
{
        ...
        if ($this->request->isPost()) {
            ...
            if (!$validate->check($data)) {
                ...
                if ($model->id) {
                    ...
                    $this->success(lang('Add success!'), url('article.index/index'));
                } else {
    ...
    public function edit($id)
    {
        ...
        if ($this->request->isPost()) {
            ...
            if (!$validate->check($data)) {
                ...
                $this->success(lang('Update success!'), url('article.index/index'));
            }
        }
    ...
    public function delete($id)
    {
        ...
        if ($result) {
            ...
            $this->success(lang('Delete success!'), url('article.index/index'));
        } else {
    ...
    public function status($id, $status = 0)
    {
        ...
        if ($result && $data['status'] === 1) {
            ...
            $this->success("发布成功", url('article.index/index'));
        } elseif ($result && $data['status'] === 0) {
            user_log($this->mid, 'cancelarticle', 1, '撤销文章 ' . $id, 'manager');
            $this->success("撤销成功", url('article.index/index'));
        } else {
            $this->error("操作失败");
        }
    }
    ...
    public function imageadd($aid)
    {
        if ($this->request->isPost()) {
            ...
            if (!$validate->check($data)) {
                ...
                $model = Db::name("ArticleImages");
                $url = url('article.index/imagelist', array('aid' => $aid));
    ...
    public function imageupdate($id)
    {
        ...
        if ($this->request->isPost()) {
            ...
            if (!$validate->check($data)) {
                ...
                $model = Db::name("ArticleImages");
                $url = url('article.index/imagelist', array('aid' => $data['article_id']));
                $delete_images = [];
                $uploaded = $this->_upload('article', 'upload_image');
                if (!empty($uploaded)) {
                    $data['image'] = $uploaded['url'];
    ...
    public function imagedelete($aid, $id)
    {
        ...
        if ($result) {
            $this->success(lang('Delete success!'), url('article.index/imagelist', array('aid' => $aid)));
        } else {
            $this->error(lang('Delete failed!'));
        }
    }
    ...
    public function commentstatus($id, $type = 1)
    {
        ...
        if ($result && $data['status'] === 1) {
            ...
            $this->success("审核成功", url('article.index/comments'));
        } elseif ($result && $data['status'] === 2) {
            user_log($this->mid, 'hidecomment', 1, '隐藏评论 ' . $id, 'manager');
            $this->success("评论已隐藏", url('article.index/comments'));
        } else {
            $this->error("操作失败");
        }
    }
    ...
    public function commentdelete($id)
    {
        ...
        if ($result) {
            ...
            $this->success(lang('Delete success!'), url('article.index/comments'));
        } else {
            $this->error(lang('Delete failed!'));
        }
    }
