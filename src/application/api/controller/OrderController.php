...
class OrderController extends AuthedController
{
    ...
    public function confirm($from = 'quick')
    {
        ...
        $data = $this->request->only('address_id,pay_type,debit_type,debit_amount,remark,subscribe,total_price,total_postage', 'put');
        ...
        if (!$validate->check($data)) {
            ...
            $balancepay = 0;
            if (!empty($data['debit_type'])) {
                if ($data['debit_type'] == 'balance' || $data['debit_type'] == 'money') {
                    $balancepay = 1;
                }
                if ($data['debit_type'] == 'reward') $balancepay = 'reward';
